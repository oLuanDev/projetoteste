...
import { Candidate, AIAnalysis, Job } from '../types';
...
export const getDecisionSupportSummary = async (candidate: Candidate, job: Job, feedbackNotes: string): Promise<string | null> => {
  const prompt = `
    ...
    **Sua Tarefa:**
    Com base **exclusivamente** nas anotações do recrutador, gere uma análise objetiva em um único parágrafo para auxiliá-lo na decisão. Sua análise deve:
    1.  Ponderar os pontos fortes e os pontos a desenvolver que foram mencionados.
...
export const summarizeInterviewFeedback = async (candidate: Candidate, job: Job, feedbackNotes: string, decision: 'offer' | 'rejected' | 'waitlist'): Promise<string | null> => {
  ...
  const prompt = `
    ...
    **Sua Tarefa:**
    Atuando como um especialista em RH, gere um parágrafo conciso e profissional que resuma a razão principal para a decisão de ${decisionText} este candidato. 
...
export const getAIResponseForChat = async (prompt: string, jobs: Job[], candidates: Candidate[]): Promise<string | null> => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: `
          **Persona:** Você é a "Assistente IA", a assistente de recrutamento virtual do restaurante Lacoste Burger. Sua missão é ser uma ferramenta indispensável para o recrutador, fornecendo insights, automatizando tarefas e oferecendo conselhos estratégicos. Você é profissional, prestativa, e expert em RH.
