...
type StatusFilter = 'all' | 'approved' | 'rejected' | 'applied' | 'screening';
...
const JobDetails: React.FC<JobDetailsProps> = ({ jobId, onBack, jobs, candidates, users, onUpdateCandidate, onArchiveCandidate, onRestoreCandidate, onPermanentDeleteCandidate, onInterviewScheduled, onBulkInterviewScheduled, onAddCandidates }) => {
  ...
  const scoredCandidates = useMemo(() => {
    ...
    return activeApplicants.map(candidate => {
        const required = aiConfig.requiredKeywords.toLowerCase().split(',').map(k => k.trim()).filter(k => k);
        const combinedText = `${candidate.skills.join(', ')} ${candidate.summary}`.toLowerCase();
        const matchedRequiredCount = required.length > 0 ? required.filter(keyword => combinedText.includes(keyword)).length : 0;
        const requiredMatchScore = required.length > 0 ? (matchedRequiredCount / required.length) * 10 : 10;
        // FIX: The right-hand side of an arithmetic operation must be of type 'any', 'number', 'bigint' or an enum type.
        // Coercing fitScore to a number prevents errors if it's not a number type (e.g., a string).
        const baseScore = Number(candidate.fitScore ?? 5);
  ...
  const statusCounts = useMemo(() => {
    return {
        ...
        applied: scoredCandidates.filter(c => c.status === 'applied').length,
        screening: scoredCandidates.filter(c => c.status === 'screening').length,
        approved: scoredCandidates.filter(c => c.status === 'approved').length,
  ...
  }, [scoredCandidates, statusFilter, sortConfig]);
  ...
  if (!job) {
    ...
    const screenableApplicants = activeApplicants.filter(c => c.status === 'applied');
 ...
 const handleApplyAIScreening = () => {
    ...
    activeApplicants.forEach(candidate => {
        ...
        if (screeningScore >= aiConfig.minScore) {
            onUpdateCandidate({ ...candidate, status: 'approved' });
        } else if (aiConfig.autoReject) {
            onUpdateCandidate({ ...candidate, status: 'rejected' });
        }
    });
 ...
 const handleUndoScreening = () => {
    activeApplicants.forEach(candidate => {
        if (candidate.status === 'approved' || candidate.status === 'rejected') {
            onUpdateCandidate({ ...candidate, status: 'applied' });
        }
    });
                ...
                {showAIPanel && (
                    <div className="space-y-4 animate-fade-in">
                        ...
                        <div className="pt-4 border-t border-light-border dark:border-border">
                            <h4 className="font-semibold text-light-text-primary dark:text-text-primary">Previsão da Triagem <span className="text-xs text-light-text-secondary dark:text-text-secondary">({screeningPreview.total} aplicaram)</span></h4>
        ...
        <div className="lg:col-span-2">
           ...
           <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
               <div className="flex gap-2 flex-wrap">
                   ...
                   <FilterButton label="Triagem" count={statusCounts.screening} isActive={statusFilter === 'screening'} onClick={() => setStatusFilter('screening')} />
               ...
               <div className="flex gap-2 items-center flex-wrap">
                    ...
                    >
                     ...
                     {job.sources.some(s => s.name === 'Google Forms') && (
                        <button 
                            type="button"
                            onClick={() => setIsImportModalOpen(true)}
                            className="flex items-center gap-2 bg-light-surface dark:bg-surface text-light-text-secondary dark:text-text-secondary px-3 py-2 text-sm rounded-lg border border-light-border dark:border-border hover:bg-light-border dark:hover:bg-border"
                        >
