...
const HiresReports: React.FC<HiresReportsProps> = ({ candidates, jobs, allCandidates }) => {
    ...
    const timeToHireByJobData = useMemo(() => {
        ...
        candidates.forEach(c => {
            // FIX: The type of `jobTitle` was being inferred incorrectly as 'unknown'.
            // Explicitly casting the result of `jobMap.get` to `string | undefined` ensures
            // that `jobTitle` is a valid string index for `jobData`.
            const jobTitle = (jobMap.get(c.jobId) as string | undefined) || 'Outros';
            if (!jobData[jobTitle]) {
