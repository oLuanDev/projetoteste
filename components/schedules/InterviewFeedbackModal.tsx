...
const InterviewFeedbackModal: React.FC<InterviewFeedbackModalProps> = ({ isOpen, onClose, candidate, job, onUpdateCandidate }) => {
    ...
    const handleGenerateSupport = async () => {
        ...
        try {
            const summary = await getDecisionSupportSummary(candidate, job, userFeedbackText);
            if (summary) {
    ...
    const handleSave = async (newStatus: 'offer' | 'rejected' | 'waitlist') => {
        ...
        try {
            const summary = await summarizeInterviewFeedback(candidate, job, userFeedbackText, newStatus);
            if (summary) {
                finalNotes += `\n\n**Sumário da IA:**\n${summary}`;
            }
        } catch (e) {
